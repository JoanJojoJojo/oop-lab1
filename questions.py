...
class Student:
    def __init__(self, name, age, student_id):
        ...
        self.enrolled_courses = []  # store course names or Course objects
...
s1.drop("Physics")
s1.drop("History")  # edge case: not enrolled
...
class Library:
    total_books = 0  # class variable to track sum of all copies in library

    def __init__(self):
        # store books by title for simplicity
    ...
    def return_book(self, title):
        if title not in self.books:
            # allow returning a book not present previously: add it with 1 copy
...
lib.borrow_book("Python Programming")  # try to borrow when not available

lib.return_book("Python Programming")
lib.return_book("A Book Not Present")  # returning a book not in library
...
order = Order()
order.add_item(d1)
order.add_item(d2)  # unavailable
order.add_item(f1)
order.add_item(f2)

order.remove_item("Pizza")  # not in order
...
# simulate turning them on and off
controller.turn_on_device("LivingRoomLight")
controller.turn_on_device("KitchenFan")
controller.turn_off_device("BedroomAC")  # currently off
controller.turn_on_device("NonExisting")  # device not found
